#!/usr/bin/env python3
import subprocess,sqlite3,os,re,logging,whisper
...
def get_word_timestamps(audio_path):
    ...
    for seg in result["segments"]:
        for w in seg.get("words",[]):
            wt=clean(w["word"]).upper()
            wt=fix_names(wt)
            if wt:
                words.append({"word":wt,"start":round(w["start"],2),"end":round(w["end"],2)})
    log.info(f"  {len(words)} words")
    return words
...
def apply_overlay(vp,ap,out,hook,cta):
    words=get_word_timestamps(ap)
...
def process_videos(limit=2):
    ...
    for r in rows:
        ...
        log.info(f"  {title[:60]}")
        if apply_overlay(base_vp,ap,out,hook,cta):
            conn=get_db(); c=conn.cursor()
