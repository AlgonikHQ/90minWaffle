...
def build(groups, hook, cta, duration, fmt="F2", title=""):
    ...
    # SUBSCRIBE prompt — yellow, bold, above handles
    parts.append(
        f"drawtext={f}text=SUBSCRIBE FOR DAILY FOOTBALL TAKES:"
        f"fontsize=26:fontcolor={YELLOW}:"
        f"x=(1080-text_w)/2:y=1715:"
        f"bordercolor=black:borderw=2:"
        f"shadowcolor=black:shadowx=2:shadowy=2"
    )

    # Red dot separators between social handles
    parts.append(
        f"drawtext={f}text=@90minWaffle  •  YouTube  •  TikTok:"
        f"fontsize=28:fontcolor={WHITE}:"
        f"x=(1080-text_w)/2:y=1755:"
        f"shadowcolor=black:shadowx=2:shadowy=2"
    )
...
def process_videos(limit=2):
    ...
    for r in rows:
        ...
        if apply_overlay(base, ap, out, hook_t, cta_t,
            ...
            # Generate thumbnail from frame at 0.5s
            thumb_path = out.replace("_overlay.mp4", "_thumb.jpg")
            try:
                thumb_result = subprocess.run(
                    ["ffmpeg", "-y", "-ss", "0.5", "-i", out,
                     "-vframes", "1", "-q:v", "2", thumb_path],
                    capture_output=True, text=True
                )
                if thumb_result.returncode == 0 and os.path.exists(thumb_path):
                    log.info(f"  Thumbnail: {thumb_path} ({os.path.getsize(thumb_path)//1024}KB)")
                else:
                    thumb_path = None
            except Exception as e:
            ...
            c    = conn.cursor()
            if thumb_path:
                c.execute("UPDATE stories SET video_path=?, thumbnail_path=?, status='queued' WHERE id=?", (out, thumb_path, sid))
            else:
