...
def best_team_image(team: dict) -> Optional[str]:
    """Badge > logo > banner > fanart > stadium thumb. None if all missing."""
    return (
        team.get("badge")
        or team.get("logo")
        or team.get("banner")
...
def find_player_image(player_name: str, team_id: str) -> Optional[str]:
    ...
    for p in roster:
        if _normalize(p.get("name", "")) == pn:
            return best_player_image(p)
    # Substring match (handles "Salah" vs "Mohamed Salah")
    for p in roster:
        if pn in _normalize(p.get("name", "")):
            return best_player_image(p)
    return None
...
def extract_player_names(text: str, limit: int = 5) -> List[str]:
    """Pull plausible 'First Last' name candidates from free text.

    Strategy: find every capitalised word, then form overlapping pairs of
    consecutive capitalised words. Reject any pair where either word is a
    known team-name token (so 'Newcastle Declan' → rejected, but the next
    pair 'Declan Rice' → accepted).
    """
    if not text:
        return []
